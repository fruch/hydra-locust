...
def main():
    ...
    def set_connection_header(request, operation_name, **kwargs):  # pylint: disable=unused-argument
        request.headers['Connection'] = 'keep-alive'

    session = boto3.Session()
    session.events.register('request-created.dynamodb', set_connection_header)

    config = Config(signature_version=botocore.UNSIGNED, max_pool_connections=10,
                    inject_host_prefix=False, parameter_validation=False)
    dynamodb = session.resource('dynamodb', endpoint_url='http://127.0.0.1:8080', config=config)
